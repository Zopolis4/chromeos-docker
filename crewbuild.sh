...
docker run --platform ${PLATFORM} --rm --net=host ${PAGER_PASSTHROUGH} ${X11} -e LOCALRC="${LOCALRC}" -v $(pwd)/pkg_cache:/usr/local/tmp/packages --v $(pwd):/output -h $(hostname)- -it ${REPOSITORY}/crewbuild:${name}-.m${milestone}
